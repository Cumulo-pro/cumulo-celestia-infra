# Celestia Fibre Server Setup (mocha-5)
...
This document records how Cumulo set up Fibre for its **mocha-5** validator, ahead of and through the Mocha v10 activation (block `1082619`, 24 Sept 2026).
...
**Important:** the app-to-fibre link (`--app-grpc-address`) and the signer link (`--signer-grpc-address`) are **not TLS-protected**. They must stay on loopback or a trusted private network. Only the Fibre client port (`7980` by default) is meant to be publicly reachable, and its transport is TLS 1.3, automatic and certificate-free (the server mints its own cert at each startup, endorsed by the validator's consensus key).
...
Our mocha-5 validator host is the one that actually holds the bonded validator (`BOND_STATUS_BONDED`, local `priv_validator_key.json`, no remote signer), so it was the natural, and per the above the only currently-recommended, choice, as opposed to e.g. a Bridge/RPC/Snapshot host, which doesn't hold the consensus key at all.

**Host specs:**

- AMD Ryzen 9 7950X3D, 16 cores / 32 threads @ 4.20GHz (5.70GHz turbo)
- 128GB DDR5 ECC RAM
- 2x 960GB NVMe (no RAID)

## Prerequisites checklist

| Requirement | Status |
|---|---|
| CPU passes `tools/cpu_requirements` (128MB/6s upgrade) | Confirmed, see benchmark below |
| 32GB RAM minimum | Confirmed, 128GB available |
| Validator bonded (storage budget derives from stake) | Confirmed |
| Disk/bandwidth sized to stake | Confirmed, see sizing below |
| App gRPC enabled (`127.0.0.1:9090`) | Confirmed |
| Privval gRPC enabled | Confirmed |
| Fibre listen port (`7980`) reachable | Confirmed |
| Separate disks for Fibre data and celestia-app data | Confirmed, see disk layout below |

### CPU benchmark

The 128MB/6s upgrade that Fibre requires is gated on execution time, not raw core count. The tool's hardware fallback recommendation (32+ cores, GFNI, SHA-NI) only applies if the benchmark itself comes out slower than reference; it is not a fixed floor.

Our host has only 16 physical cores (32 via SMT), below that fallback recommendation, but the actual benchmark (`go run ./tools/cpu_requirements`, run from a `v10.2.0-mocha` checkout) passed comfortably:
...
| Operation | Result | Reference | Outcome |
|---|---|---|---|
...
The host has two physical NVMe disks, no RAID:

```
disk 1 (root)          ->  celestia-appd home
disk 2 (second NVMe)    ->  dedicated to Fibre
```

The second disk previously hosted an unrelated Dymension mainnet validator. It was migrated off to another host in Cumulo's fleet to isolate Fibre's I/O from anything non-Celestia-related, and the node data (down to a kept operator wallet) was cleaned up afterwards, leaving the disk effectively free.

One important correction made during this audit: a 159GB `avail` directory on that same disk was initially assumed to be a stale backup, but turned out, after confirming a matching device and inode between the directory and a symlink used by the running process, to be the live, in-production Avail mainnet RPC node data directory. It was left untouched. It holds no validator keys (confirmed with a recursive search for keyring/priv_validator files), so it remains a low-risk candidate for future I/O isolation, but at 676GB free even with it in place (against Fibre's actual ~88.6GB need) there was no pressing need to move it.
...
**`config/config.toml`**, privval gRPC. In our case this was already configured, just on a non-default port:
```toml
...
- `--signer-grpc-address 127.0.0.1:26659` matches our host's actual (non-default) privval port, see above.
- `--max-connections 32` (default is `16`). The Fibre README notes that a single upload uses 16 signers and therefore occupies all 16 connection slots by default, blocking concurrent downloads for its duration. Worst-case memory is `max_connections x max_concurrent_streams x 132 MiB`; at `32 x 13` that's roughly 53.5GiB, comfortably inside 128GB.
- `--otel-endpoint http://127.0.0.1:4318`, see Monitoring below.
...
While reviewing the firewall for this, four allowed ports turned out to have no process listening behind them at all, checked against `ss -tlnp`/`ss -ulnp` and cross-referenced with everything actually running on the host. They were most likely leftovers from the now-migrated Dymension validator and were removed:

```bash
sudo ufw delete allow <stale-port-1>
sudo ufw delete allow <stale-port-2>
sudo ufw delete allow <stale-port-3>
sudo ufw delete allow <stale-port-4>
```
...
celestia-appd tx valaddr set-host <public_ip>:7980 \
...
As of 24 Sept 2026: all pre-activation prerequisites are complete (binary installed, config staged, systemd units in place but not started, firewall opened, monitoring pipeline verified end to end). Fibre itself has not been started yet; that, along with `set-host` registration, happens once Mocha's v10 activation height is confirmed crossed.

Open items:
- Revisit whether to move the Avail RPC node to another disk for I/O isolation (not urgent, disk headroom is ample either way).
- No RAID on this host: a disk failure would take Fibre's shard store with it (not consensus state, recoverable, but affects service continuity). Worth a decision on whether to mitigate.
- Repeat this setup for Cumulo's mainnet validator once Fibre is required there.
